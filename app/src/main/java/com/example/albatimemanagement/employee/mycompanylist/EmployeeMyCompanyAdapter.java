package com.example.albatimemanagement.employee.mycompanylist;
...
public class EmployeeMyCompanyAdapter extends ArrayAdapter<CompanyInfo> {
    private Context context;
    private List mList;

    class AccountViewHolder {
        private TextView tvItemEmployeeCompanyName;
        private TextView tvItemEmployeePresidentName;
    }


    // VocaAdapter를 초기화하기 위한 생성자로 아규먼트로 전달받은 context와 list를 내부 저장 공간에 저장합니다.
    public EmployeeMyCompanyAdapter(Context context, // 레이아웃 XML 파일을 뷰(view) 객체로 바꾸는데 사용할 컨택스트(Context)
    ...
    public View getView(int position, View convertView, @NonNull ViewGroup parentViewGroup) {
        ...
        if (rowView == null) {
            rowView = LayoutInflater.from(getContext()).inflate(R.layout.item_employee_account_company_list, parentViewGroup, false);
            ...
            viewHolder.tvItemEmployeeCompanyName = rowView.findViewById(R.id.tvItemEmployeeCompanyName);
            viewHolder.tvItemEmployeePresidentName = rowView.findViewById(R.id.tvItemEmployeePresidentName);
        ...
        viewHolder.tvItemEmployeeCompanyName.setText(c.getName());
        viewHolder.tvItemEmployeePresidentName.setText(c.getPresidentName());
